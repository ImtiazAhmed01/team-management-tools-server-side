  ...
  try {
    ...
    app.patch("/approveLeader/:email", async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount > 0 && requestResult.modifiedCount > 0) {
          res
            .status(200)
            .json({ message: "User role updated and request approved" });
        } else {
          res
            .status(400)
            .json({ message: "Failed to approve user role or request" });
        }
      } catch (error) {
        console.error(error);
    ...
    app.patch("/declineLeader/:email", async (req, res) => {
      ...
      try {
        ...
        if (requestResult.modifiedCount > 0) {
          res
            .status(200)
            .json({ message: "User role updated and request declined" });
        } else {
          res
            .status(400)
            .json({ message: "Failed to decline user role or request" });
        }
      } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Server error" });
    ...
    app.put("/mytasks/:id", async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ error: "Failed to update user task", details: err });
      }
    });
    ...
    app.get("/userassignedtasks/:email", async (req, res) => {
      ...
      try {
        const userTasks = await userTaskCollection
    ...
    app.put("/task/:id", async (req, res) => {
      ...
      try {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          console.log(
            "🔧 Fixed non-numeric fields before incrementing:",
            updates
          );
        }
        ...
        res
          .status(500)
          .send({ error: "Failed to update task counts", details: err });
      }
    });
    ...
    app.patch("/user/:email", async (req, res) => {
      ...
      try {
        const result = await userCollection.updateOne(
          { email },
          { $set: { photoURL: image, userImage: image } }
        );
