...
// Configure Socket.IO with dynamic CORS for development and production
...
const uri = `mongodb+srv://${process.env.DB_user}:${process.env.DB_pass}@cluster0.khtuk.mongodb.net/collabnesttools?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post('/upload-image', async (req, res) => {
      ...
      try {
        const result = await imageCollection.insertOne(task);
      ...
      try {
        ...
        if (isExist) {
          ...
          if (updatedProfile.modifiedCount > 0) {
            ...
            const updatedData = await profileCollection.findOne({ email });
            res.status(200).json({
              success: true,
          ...
          if (newProfile.insertedId) {
            res.status(201).json({
              ...
              user: await profileCollection.findOne({
                _id: newProfile.insertedId,
              }),
            });
          } else {
            res.status(400).json({
    ...
    app.patch("/userImage/:email", async (req, res) => {
      try {
        const email = req.params.email;
        ...
        const userResult = await userCollection.updateOne(
          { email },
          { $set: { userImage: image, photoURL: image } }
        );
